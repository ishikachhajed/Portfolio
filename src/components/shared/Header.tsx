...
import { PiSunDuotone, PiMoonDuotone, PiList, PiX } from "react-icons/pi";
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-4 flex items-center justify-end">
        ...
        <nav aria-label="Primary" className="relative flex items-center gap-2 sm:gap-3">
          ...
          >
            {mobileMenuOpen ? <PiX size={20} /> : <PiList size={20} />}
          </button>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="sm:hidden absolute top-full left-0 right-0 bg-[var(--surface)] border-b border-[var(--border)] shadow-lg px-6 py-4 flex flex-col gap-4 z-40"
            ...
            {links.map((l) => (
              <a
                ...
                className={`text-lg font-medium ${
                  active === l.href ? "text-[var(--brand)]" : "text-[var(--text)]"
                }`}
              >
                {l.label}
              </a>
