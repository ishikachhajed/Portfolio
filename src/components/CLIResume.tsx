...
export default function CLIResume({ open = false, onClose }: Props) {
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (processing) {
      if (e.key === "Tab") {
        ...
        e.preventDefault(); // don't run a new command while running
      }
      return;
    }
  ...
  return (
    <>
      ...
      <div
        ref={panelRef}
        className={`fixed z-60 bottom-4 w-full max-w-4xl left-1/2 transform -translate-x-1/2 px-4 pointer-events-none`}
        role="dialog"
        aria-modal="true"
      >
        <div
          className={`pointer-events-auto w-full max-w-4xl transition-all transform
            ${
              fullscreen
                ? "h-full mt-0 rounded-none"
                : minimized
                  ? "h-12"
                  : "h-[60vh] sm:h-[420px] rounded-xl"
            ...
            {!minimized && (
              ...
              >
                ...
                <input
                  ...
                  placeholder={
                    processing ? 'Press Tab to skip animation' : 'type "help" and press Enter'
                  }
                ...
                <button
                  ...
                  style={{
                    background: processing || input.trim() === "" ? "var(--border)" : "var(--brand)",
                    color: "white",
